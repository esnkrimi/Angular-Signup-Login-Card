...
export class EsnSignupLoginComponent implements OnInit {
  ...
  login() {
    this.service.login(this.data.loginApiHref, this.loginForm.value).subscribe(d => {
      this.callbackLogin.emit(d);
    })
  }

  signup() {
    this.service.signup(this.data.sigunpApiHref, this.signupForm.value).subscribe(d => {
      this.callbackSignup.emit(d);
    })
  }
}